(function() {
  ...
  function pickOwnServer(callback) {
    function next(i) {
      if (i >= MY_API_SERVERS.length) return callback(MY_API_SERVERS[0]);
      ownPing(MY_API_SERVERS[i], function(ok) {
        if (ok) return callback(MY_API_SERVERS[i]);
        next(i + 1);
      });
    }
    next(0);
  ...
  // Choose a reachable backend once per session.
  pickOwnServer(function(server) {
    MY_API = server;
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent(MY_API.indexOf(location.host) >= 0 ? MY_API : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke(MY_API.replace(/\/$/, ''), function() {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript([MY_API + "js/nws-client-es5.js?v03022026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  var ZPREM_SERVER = MY_API;
  var ZPREM_CHECK_URL = MY_API + 'premium/check';
  var ZPREM_PAY_URL = MY_API + 'premium/pay';
  var ZPREM_TRIAL_URL = MY_API + 'premium/trial';
  ...
  function startPlugin() {
    ...
    function zpremActivate() { return; }

    function zpremCheck(callback) { if (callback) callback(false); }

    function zpremTrial(callback) { if (callback) callback(false, 'disabled'); }

    zpremActivate();
    // Independent build: no Z01 premium checks.
